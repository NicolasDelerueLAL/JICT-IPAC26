...
$content .= "<h3>Information about reviewer ".$queryArray["person_id"]."</h3><BR/>\n";
...
$content .= "<BR/><BR/>\n";
if ($queryArray["confirm"]==1){
    //$content .= assign_reviewer_to_paper($queryArray["contribution_id"], $the_reviewer["user_id"]);
    comment_paper($queryArray["contribution_id"],"Inviting reviewer ".$the_reviewer["user_id"]);
    send_email_file_to_eventperson("message_referee_request.txt","EventPerson:".$the_reviewer["id"],"delerue@lal.in2p3.fr",true,$contributions[$queryArray["contribution_id"]]);
    $content .= "<BR/><BR/>\n";
    $content .= "<A HREF='list_papers.php'>Go to the list of papers</A><BR/><BR/>\n";
    $content .= "<A HREF='list_participants.php'>Go to the list of participants</A><BR/><BR/>\n";
    ...
    $content .= "<BR/><BR/>\n";
    $content .= "<HR>\n";
    $content .= "<b> Confirm assignation? <A HREF='assign_paper.php?contribution_id=".$queryArray["contribution_id"]."&person_id=".$queryArray["person_id"]."&confirm=1'>Yes</A> | <A HREF='edit_paper.php?contribution_id=".$queryArray["contribution_id"]."'>No</A></b>\n";
} 
$content .= "<BR/><BR/>\n";
$content .= "<BR/><BR/>\n";
$content .= "<BR/><BR/>\n";
